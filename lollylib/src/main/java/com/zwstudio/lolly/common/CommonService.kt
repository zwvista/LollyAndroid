...
fun Single<List<List<MSPResult>>>.logCreateResult(): Single<Int> =
    map {
        logDebug("Created new item, result=$it")
        it[0][0].newid!!.toInt()
    }

fun Single<List<List<MSPResult>>>.logUpdateResult(id: Int): Completable =
    flatMapCompletable {
        logDebug("Updated item ID=${id}, result=$it")
        Completable.complete()
    }

fun Single<List<List<MSPResult>>>.logDeleteResult(): Completable =
    flatMapCompletable {
        logDebug("Deleted item, result=$it")
        Completable.complete()
    }
