...
fun PhrasesUnitBatchEditScreen(vm: PhrasesUnitViewModel, navController: NavHostController?) {
    ...
    val isCheckedList = remember { mutableStateListOf(*Array(vm.lstPhrases.size) { false }) }
    Column(modifier = Modifier.fillMaxSize()) {
        TopBarArrow(
            ...
            actions = {
                Button(
                    onClick = {
                        isCheckedList.forEachIndexed { index, item -> vm.lstPhrases[index].isChecked = item }
                        vmBatchEdit.save()
                    },
        ...
        Column(
            ...
            if (vm.isBusy) {
                ...
                LazyColumn(
                    ...
                    itemsIndexed(vm.lstPhrases, key = { _, item -> item.id }) { index, item ->
                        Card(
                            modifier = Modifier
